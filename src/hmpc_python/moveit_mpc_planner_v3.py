...
import numpy as np
import casadi as ca
from mpc_planner_v3 import MPCPlanner  # 确保你已经创建了 MPCPlanner 类
...
class MoveItMPCPlanner(Node):
    ...
    def publish_trajectory(self, X_opt):
        """发布 MPC 计算出的最优轨迹，并可视化关节变化情况"""
        self.get_logger().info("准备发布 MPC 轨迹...")

        # 可视化 9 个关节随时间变化的轨迹
        timesteps = np.arange(X_opt.shape[1])  # 时间步
        ...
        joint_trajectory_msg.header.frame_id = "world"  # 参考坐标系
