...
class HMPCPlanner_task2:
    def __init__(self, logger, urdf_path,valid_steps):
        self.logger = logger
        self.logger.info("初始化 HMPC 规划器 task2 完整版")
        ...
        #暂时现在是
        continuity_arm=200
        self.Q_continuity=ca.diagcat(continuity_base, continuity_base, continuity_base, continuity_arm, continuity_arm, continuity_arm, continuity_arm, continuity_arm, continuity_arm)  # 连续性惩罚的对角矩阵
        acc_arm=1200
        self.Q_acceleration = ca.diag(ca.DM([acc_arm] * 9))
        ...
        self.Q_key_points = [
            ...
            ca.diagcat(key_points_weight, key_points_weight, key_points_weight, key_points_weight, key_points_weight,
                       key_points_weight),  # 关键点 1 的权重矩阵
        ]

        # # 定义关键点和它们对应的时间步
        # self.key_points = np.array([
        #     [-1.15, -0.169, 0.021, -0.025, -0.212, -0.5],
        #     [-1.986, -0.119, 0.098, -0.074, -0.206, -0.5],
        #     [-2.703, -0.069, 0.174, -0.122, -0.201, -0.5],
        #     [-3.047, -0.063, 0.173, -0.123, -0.196, -0.5]
        # ])
        #
        #
        # # 定义每个关键点对应的时间步
        # self.key_point_steps = [8, 15, 20, 25]  # 这些关键点应该在时间步5, 10, 15, 20, 25, 30处被应用
        # 关键点和时间步
        self.key_points = np.array([
        ...
        self.key_point_steps = [5,8, 15, 20, 25]  # 对应时间步
    ...
    def build_cost_function_follow(self):
        ...
        # 添加连续性惩罚
        continuity_penalty = 0  # 初始化连续性惩罚项
        # 假设你想对第0步的惩罚加权，比如加大10倍
        alpha = 10.0
        # 先惩罚第 0 步和 0 之间的跳变（如果你默认U[0]来自0）
        delta_u0 = self.U[:, 0]  # 默认前一步为0
        continuity_penalty += alpha * ca.mtimes(delta_u0.T, self.Q_continuity @ delta_u0)
        ...
        # k = 0
        a2=10
        delta2_u0 = self.U[:, 1] - 2 * self.U[:, 0]
        acceleration_penalty += a2*ca.mtimes(delta2_u0.T, self.Q_acceleration @ delta2_u0)

        # k = 1
        delta2_u1 = self.U[:, 2] - 2 * self.U[:, 1] + self.U[:, 0]
        acceleration_penalty += a2*ca.mtimes(delta2_u1.T, self.Q_acceleration @ delta2_u1)

        # k >= 2
        for k in range(2, self.N - 1):
            delta2_u = self.U[:, k + 1] - 2 * self.U[:, k] + self.U[:, k - 1]
            acceleration_penalty += ca.mtimes(delta2_u.T, self.Q_acceleration @ delta2_u)

        self.cost_fn += acceleration_penalty
    ...
    def follow_adjust_matrix(self, current_ee_pose, target_ee_pose):
        ...
        terminal=10000
        terminal_ori=terminal/5
        self.Q_terminal = ca.diagcat(terminal, terminal, terminal, terminal_ori, terminal_ori, terminal_ori, terminal_ori)  # 强化终端误差优化