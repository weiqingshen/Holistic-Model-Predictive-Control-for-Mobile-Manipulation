...
from mpc_planner_v18 import MPCPlannerV18
import numpy as np
import casadi as ca
import time
import sys
...
class MoveItMPCPlannerv18(Node):
    def __init__(self, target_pose=None):
        super().__init__('moveit_mpc_planner_v18')
        ...
        # 初始化 MPC 优化器
        self.mpc = MPCPlannerV18(self.get_logger(), self.urdf_path,self.valid_steps )
        ...
        self.get_logger().info("MoveIt MPC Planner v18 启动完成")
    ...
    def publish_trajectory(self):
        ...
        key_points = np.array([
            ...
            [-2.803, -0.069, 0.174, -0.122, -0.201, -0.5],
            [-3.247, -0.063, 0.173, -0.123, -0.196, -0.5]
        ])

        key_point_steps = [8, 15, 20, 25]  # 对应时间步

        X_opt_data = ca.horzcat(self.X_opt_all, self.X_opt_follow)
        # 时间步
        timesteps = np.arange(X_opt_data.shape[1])
        # === 绘制轨迹图 ===
        # 关节标签
        ...
        for i, step in enumerate(key_point_steps):
            for j in range(3, 9):  # 仅标注后6个关节
                plt.scatter(step + self.X_opt_all.shape[1], key_points[i, j - 3], color=colors[j], marker='o')  # 使用对应关节的颜色
...
def main(args=None):
    ...
    # 初始化节点
    node = MoveItMPCPlannerv18(target_pose)
