...
from mpc_planner_v24 import MPCPlannerV24
import numpy as np
import casadi as ca
import time
import sys
...
from spherical_decomposition.final_main_v4 import symbolic_collision_analysis

class MoveItMPCPlannerv24(Node):
    def __init__(self, target_pose=None):
        super().__init__('moveit_mpc_planner_v24')
        ...
        # 初始化 MPC 优化器
        self.mpc = MPCPlannerV24(self.get_logger(), self.urdf_path,self.valid_steps )
        ...
        self.get_logger().info("MoveIt MPC Planner v24 启动完成")
    ...
    def run_mpc(self):
        ...
        # 执行符号碰撞分析
        link_pair_counts, symbolic_result = symbolic_collision_analysis(joint_angles)
        ...
        self.mpc.set_symbolic_collision_constraints(symbolic_result, danger_threshold=0.08, a=100, b=120)
        # # 动态调整 R 矩阵
    ...
    def publish_trajectory(self):
        ...
        threshold_position_error = 0.03  # 位置误差阈值 (可以调整)
        threshold_quaternion_error = 0.21  # 四元数误差阈值 (可以调整)
        n = 0
    ...
    def run_mpc_dynamic(self):
        ...
        # 设置符号碰撞代价
        # self.mpc.set_symbolic_collision_constraints(symbolic_result, danger_threshold=0.08, a=100, b=500)
        self.mpc.set_symbolic_collision_constraints(symbolic_result, danger_threshold=0.08, a=1, b=120)
        # # 动态调整 R 矩阵
        # self.mpc.adjust_R_matrix(ee_pose_np, target_ee_pose)
        self.mpc.build_cost_function()
        self.mpc.set_optimize_option()
...
def main(args=None):
    ...
    # 初始化节点
    node = MoveItMPCPlannerv24(target_pose)
