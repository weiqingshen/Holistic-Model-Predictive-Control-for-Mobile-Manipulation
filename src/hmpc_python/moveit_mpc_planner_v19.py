...
from mpc_planner_v19 import MPCPlannerV19
import numpy as np
import casadi as ca
import time
import sys
...
class MoveItMPCPlannerv19(Node):
    def __init__(self, target_pose=None):
        super().__init__('moveit_mpc_planner_v19')
        ...
        # 初始化 MPC 优化器
        self.mpc = MPCPlannerV19(self.get_logger(), urdf_path,self.valid_steps)
        ...
        self.get_logger().info("MoveIt MPC Planner v19 启动完成")
    ...
    def run_mpc(self):
        ...
        target_ee_pose = ca.DM([
            self.target_pose.pose.position.x,
            self.target_pose.pose.position.y,
            self.target_pose.pose.position.z, #动态跟随
            self.target_pose.pose.orientation.x,
            self.target_pose.pose.orientation.y,
            self.target_pose.pose.orientation.z,
            self.target_pose.pose.orientation.w
...
def main(args=None):
    ...
    # 初始化节点
    node = MoveItMPCPlannerv19(target_pose)
