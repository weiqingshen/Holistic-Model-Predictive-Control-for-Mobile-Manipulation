...
from mpc_planner_v30 import MPCPlannerV30
import numpy as np
import casadi as ca
import time
import sys
...
class MoveItMPCPlannerv30(Node):
    def __init__(self, target_pose=None):
        super().__init__('moveit_mpc_planner_v30')
        ...
        # 初始化 MPC 优化器
        self.mpc = MPCPlannerV30(self.get_logger(), urdf_path,self.valid_steps,x_p=0.0, y_p=0.4, r=0.1)
        ...
        self.get_logger().info("MoveIt MPC Planner v30 启动完成")
...
def main(args=None):
    ...
    # 初始化节点
    node = MoveItMPCPlannerv30(target_pose)
