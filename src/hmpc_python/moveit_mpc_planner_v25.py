...
from mpc_planner_v25 import MPCPlannerV25
import numpy as np
import casadi as ca
import time
import sys
...
class MoveItMPCPlannerv25(Node):
    def __init__(self, target_pose=None):
        super().__init__('moveit_mpc_planner_v25')
        ...
        # 初始化 MPC 优化器
        self.mpc = MPCPlannerV25(self.get_logger(), self.urdf_path,self.valid_steps )
        ...
        self.get_logger().info("MoveIt MPC Planner v25 启动完成")
    ...
    def run_mpc(self):
        ...
        #
        self.mpc.set_symbolic_collision_constraints(symbolic_result, danger_threshold=0.08, a=0.5, b=120)
        # # 动态调整 R 矩阵
    ...
    def publish_trajectory(self):
        ...
        threshold_position_error = 0.01  # 位置误差阈值 (可以调整)
        threshold_quaternion_error = 0.01  # 四元数误差阈值 (可以调整)
        n = 0
    ...
    def run_mpc_dynamic(self):
        ...
        # self.mpc.set_symbolic_collision_constraints(symbolic_result, danger_threshold=0.08, a=100, b=500)
        custom_params = {
            ("link3", "obstacle"): {'danger_threshold': 0.08, 'a': 1, 'b': 200},
            ("obstacle", "link3"): {'danger_threshold': 0.08, 'a': 1, 'b': 200},
            ("obstacle", "link4"): {'danger_threshold': 0.08, 'a': 1, 'b': 200},
            ("link4", "obstacle"): {'danger_threshold': 0.08, 'a': 1, 'b': 200}
        }
        self.mpc.set_symbolic_collision_constraints(symbolic_result, danger_threshold=0.05, a=0.5, b=100,custom_params=custom_params)
        # # 动态调整 R 矩阵
        # self.mpc.adjust_R_matrix(ee_pose_np, target_ee_pose)
...
def main(args=None):
    ...
    # 初始化节点
    node = MoveItMPCPlannerv25(target_pose)
