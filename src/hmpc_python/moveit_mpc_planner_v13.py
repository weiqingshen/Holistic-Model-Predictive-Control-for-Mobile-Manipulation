...
from mpc_planner_v13 import MPCPlannerV13
import numpy as np
import casadi as ca
import time
import sys
...
class MoveItMPCPlannerv13(Node):
    def __init__(self, object_name=None):
        super().__init__('moveit_mpc_planner_v13')
        ...
        # 初始化 MPC 优化器
        self.mpc = MPCPlannerV13(self.get_logger(), urdf_path)

        self.current_state = None
        self.X_opt_all = None  # 用于存储所有的轨迹
        self.valid_steps = 10  # 有效的执行步数
        ...
        if object_name == "object1":
            self.get_logger().info("订阅 arm_pose 以获取目标 pose")
            self.pose_subscriber = self.create_subscription(
                PoseArray,
                "/arm_pose",
                self.pose_array_callback,  # 放在第三个参数
                10  # 队列深度，放在第四个参数
            )
        else:
            self.get_logger().info("使用默认目标 pose")
            self.target_pose = self.default_target_pose()
        ...
        self.get_logger().info("MoveIt MPC Planner v13 启动完成")
    ...
    def pose_array_callback(self, msg):
        # 从 PoseArray 的第一个 Pose 获取目标位置
        print("订阅消息")
        self.get_logger().info("收到 arm_pose 消息，更新目标 pose")

        pose = PoseStamped()
        pose.header.frame_id = "world"
        pose.pose.position.x = msg.poses[0].position.x
        pose.pose.position.y = msg.poses[0].position.y
        pose.pose.position.z = 0.4  # 手动设定 Z 值
    ...
    def publish_trajectory(self):
        ...
        threshold_quaternion_error = 0.001  # 四元数误差阈值 (可以调整)
    ...
    def run_mpc_dynamic(self):
        ...
        print(self.X_opt_all)
        if X_opt is not None:
            # 只将前10步添加到 X_opt_all 中
            self.X_opt_all = ca.horzcat(self.X_opt_all, X_opt[:, :self.valid_steps])
...
def main(args=None):
    ...
    if len(sys.argv) == 2:
        ...
        print(f"[main] 接收到 object 参数: {object_name}")
    else:
        print("[main] 未接收到 object 参数，将使用默认")

    # 初始化节点
    node = MoveItMPCPlannerv13(object_name)
