...
class MPCPlannerV9:
    def __init__(self, logger, urdf_path):
        self.logger = logger
        self.logger.info("初始化 MPC 规划器 v9 完整版")
        ...
        # 权重矩阵：位置 + 姿态（rpy）
        self.Q_task = ca.diagcat(500, 500, 500, 50, 50, 50, 50)
    ...
    #
    #             # 将 d_new 转换为 DM 类型并进行数值计算
    #             d_new_val = ca.DM(d_new).full()  # 使用 ca.DM 将 SX 转换为 DM，再用 .full() 获取数值
    #             #print(f"⚠️ 碰撞风险！关节对: {link_pair} | 新距离: {d_new_val[0]:.6f} | 惩罚: {penalty:.2f}")
    #
    #             # 计算惩罚项
    #             penalty = 1e4 * (0.05 - d_new_val[0]) ** 2  # 只取 d_new_val[0] 的第一个值进行计算
    #
    #             self.collision_cost_fn += penalty  # 累加碰撞代价
    #
    #             # 打印累积碰撞代价
