...
def compute_transforms(joint_angles):
    ...
    # link1的变换矩阵
    rpy1 = [0, 0, 0]
    xyz1 = [-0.000491, -0.000362, 0.044474]
    ...
    # joint1的变换矩阵
    rpy2 = [0, 0, joint_angles[0]]
    xyz2 = [0, 0, 0.1284]
    R2 = euler_to_rotation_matrix(rpy2)
    T2 = np.eye(4)
    T2[:3, :3] = R2
    T2[:3, 3] = xyz2
    T["joint1"] = T["link1"].dot(T2)

    # link2的变换矩阵
    rpy3 = [0, 0, 0]
    xyz3 = [0.006885, 0.097138, 0.013427]
    R3 = euler_to_rotation_matrix(rpy3)
    T3 = np.eye(4)
    T3[:3, :3] = R3
    T3[:3, 3] = xyz3
    T["link2"] = T["joint1"].dot(T3)

    # joint2的变换矩阵
    rpy4 = [1.5708, 0, 1.5708 + joint_angles[1]]
    xyz4 = [0, 0, 0.0927]
    R4 = euler_to_rotation_matrix(rpy4)
    T4 = np.eye(4)
    T4[:3, :3] = R4
    T4[:3, 3] = xyz4
    T["joint2"] = T["link2"].dot(T4)

    # link3的变换矩阵
    rpy5 = [0, 0, 0]
    xyz5 = [-0.016272, -0.033930, 0.008989]
    R5 = euler_to_rotation_matrix(rpy5)
    T5 = np.eye(4)
    T5[:3, :3] = R5
    T5[:3, 3] = xyz5
    T["link3"] = T["joint2"].dot(T5)

    # joint3的变换矩阵
    rpy6 = [-3.1416, 0, -1.5708 + joint_angles[2]]
    xyz6 = [0, 0.22, 0]
    R6 = euler_to_rotation_matrix(rpy6)
    T6 = np.eye(4)
    T6[:3, :3] = R6
    T6[:3, 3] = xyz6
    T["joint3"] = T["link3"].dot(T6)

    # link4的变换矩阵
    rpy7 = [0, 0, 0]
    xyz7 = [-3.1472e-05, 0.011420, 0.119063]
    R7 = euler_to_rotation_matrix(rpy7)
    T7 = np.eye(4)
    T7[:3, :3] = R7
    T7[:3, 3] = xyz7
    T["link4"] = T["joint3"].dot(T7)

    # joint4的变换矩阵
    rpy8 = [1.5708, 0, joint_angles[3]]
    xyz8 = [0, 0, 0]
    R8 = euler_to_rotation_matrix(rpy8)
    T8 = np.eye(4)
    T8[:3, :3] = R8
    T8[:3, 3] = xyz8
    T["joint4"] = T["link4"].dot(T8)

    # link5的变换矩阵
    rpy9 = [0, 0, 0]
    xyz9 = [3.9009e-05, -0.016660, 0.009755]
    R9 = euler_to_rotation_matrix(rpy9)
    T9 = np.eye(4)
    T9[:3, :3] = R9
    T9[:3, 3] = xyz9
    T["link5"] = T["joint4"].dot(T9)

    # joint5的变换矩阵
    rpy10 = [0, 0, joint_angles[4]]
    xyz10 = [0, 0, 0]
    R10 = euler_to_rotation_matrix(rpy10)
    T10 = np.eye(4)
    T10[:3, :3] = R10
    T10[:3, 3] = xyz10
    T["joint5"] = T["link5"].dot(T10)

    # link6的变换矩阵
    rpy11 = [0, 0, 0]
    xyz11 = [9.9559e-05, -2.9871e-05, 0.049717]
    R11 = euler_to_rotation_matrix(rpy11)
    T11 = np.eye(4)
    T11[:3, :3] = R11
    T11[:3, 3] = xyz11
    T["link6"] = T["joint5"].dot(T11)

    # joint6的变换矩阵
    rpy12 = [1.5708, 0, joint_angles[5]]
    xyz12 = [0, 0, 0]
    R12 = euler_to_rotation_matrix(rpy12)
    T12 = np.eye(4)
    T12[:3, :3] = R12
    T12[:3, 3] = xyz12
    T["joint6"] = T["link6"].dot(T12)

    # link_hand的变换矩阵
    rpy13 = [0, 0, 0]
    xyz13 = [0.012192, -0.001161, 0.097033]
    R13 = euler_to_rotation_matrix(rpy13)
    T13 = np.eye(4)
    T13[:3, :3] = R13
    T13[:3, 3] = xyz13
    T["link_hand"] = T["joint6"].dot(T13)

    # joint_hand的变换矩阵
    rpy14 = [0, 0, joint_angles[6]]
    xyz14 = [0, 0, 0]
    R14 = euler_to_rotation_matrix(rpy14)
    T14 = np.eye(4)
    T14[:3, :3] = R14
    T14[:3, 3] = xyz14
    T["joint_hand"] = T["link_hand"].dot(T14)

    # link_right的变换矩阵
    rpy15 = [0, 0, 0]
    xyz15 = [-0.006735, -0.005156, -0.046502]
    R15 = euler_to_rotation_matrix(rpy15)
    T15 = np.eye(4)
    T15[:3, :3] = R15
    T15[:3, 3] = xyz15
    T["link_right"] = T["joint_hand"].dot(T15)

    # joint_right的变换矩阵
    rpy16 = [-1.5708, 0, joint_angles[7]]
    xyz16 = [0, 0, 0.1465]
    R16 = euler_to_rotation_matrix(rpy16)
    T16 = np.eye(4)
    T16[:3, :3] = R16
    T16[:3, 3] = xyz16
    T["joint_right"] = T["link_right"].dot(T16)

    # link_left的变换矩阵
    rpy17 = [0, 0, 0]
    xyz17 = [0.006735, -0.005156, -0.046502]
    R17 = euler_to_rotation_matrix(rpy17)
    T17 = np.eye(4)
    T17[:3, :3] = R17
    T17[:3, 3] = xyz17
    T["link_left"] = T["joint_hand"].dot(T17)

    # joint_left的变换矩阵
    rpy18 = [-1.5708, 0, joint_angles[8]]
    xyz18 = [0, 0, 0.1465]
    R18 = euler_to_rotation_matrix(rpy18)
    T18 = np.eye(4)
    T18[:3, :3] = R18
    T18[:3, 3] = xyz18
    T["joint_left"] = T["link_left"].dot(T18)
...
def visualize_spheres_with_pybullet(T, spheres, link_names):
    ...
    # 创建一个平面作为地面
    planeId = p.loadURDF("/home/fins/myrobot_move/src/robot_arm_description/urdf/robot_arm_description.urdf")  # 使用内置的平面URDF
...
#问题：球是从link坐标系生成的（link坐标系在每个link的重心处），现在我们应该让它们从joint处开始生成
