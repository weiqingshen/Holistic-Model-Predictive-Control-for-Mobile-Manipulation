...
CUSTOM_THRESHOLDS = {
    ("link2", "link4"): 0.033,
    ("link4", "link2"): 0.033  # 加上反向，以确保无论顺序如何都能匹配
}
...
def compute_min_distances(transforms, rel_spheres, threshold=0.05):
        ...
        for j in range(i + 1, len(links)):
            ...
            for idx_a in range(len(pos_a)):
                for idx_b in range(len(pos_b)):
                    ...
                    pair_key = (a, b)
                    threshold_pair = CUSTOM_THRESHOLDS.get(pair_key, DEFAULT_THRESHOLD)
                    if dist < threshold_pair:
                        ...
                        close_pairs.append((pos_a[idx_a], pos_b[idx_b]))

    return min_distances, close_pairs
