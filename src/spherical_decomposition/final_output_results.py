...
# 将最小距离和最近球心对保存为 JSON 文件
def save_results(min_distances, closest_pairs, filename="output_results.json"):
    # 将矩阵和向量转换为可序列化格式
...
def run_pipeline(joint_angles, sphere_json_path="link_spheres_relative.json", output_path=None):
    ...
    # 只有当 output_path 明确传入时才保存结果（可选）
    if output_path:
        save_results(min_distances, closest_pairs, filename=output_path)
