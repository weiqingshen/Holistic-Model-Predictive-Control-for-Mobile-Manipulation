...
# from final_symbolic_transform_v8 import get_symbolic_transforms_of_closest_spheres,make_transform_sym
# from final_symbolic_transform_v8 import compute_transforms_symbolic_one_step
from .symbolic_transform_task1_v1 import get_symbolic_transforms_of_closest_spheres,make_transform_sym
from .symbolic_transform_task1_v1 import compute_transforms_symbolic_one_step
import casadi as ca
...
def compute_cylinder_pair_for_hand(link_name, transforms, data, cyl_center, cyl_radius, cyl_height, max_xy_error=0.05):
    ...
    for dist, T_a_to_link, T_b_to_link in valid_positions[:20]:
...
def compute_gripper_cylinder_distance_independent(transforms, data, cyl_center, cyl_radius, cyl_height, max_xy_error=0.05):
    ...
    for link_name in ['link_left', 'link_right']:
        ...
        for p in positions:
            ...
            if abs(z - cyl_center[2]) <= 0.5 * cyl_height+0.03:
                dist_xy = np.hypot(x - cyl_center[0], y - cyl_center[1])
        ...
        result[link_name] = valid_positions[:20]  # 最多返回10个
...
def run_pipeline(X,joint_angles, sphere_json_path="link_spheres_relative_v2.json", cyl_center=(0.5, 0.7, 0.035),
    ...
    result_hand=compute_cylinder_pair_for_hand('link_hand', transforms, data, cyl_center, cyl_radius, cyl_height,0.2)
